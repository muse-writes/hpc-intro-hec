...
For the HEC, these can be found on the [HEC documentation
website](https://lancaster-hec.readthedocs.io/en/main/intro.html).
...
impact using ``r config$sched$hist``. You can use this knowledge to set up the
...
use ``r config$sched$hist`` to get statistics about `parallel-job.sh`.
...
snippets('resources/account-history.Rmd')
```
...
Hint: use ``r config$sched$flag$time``.

:::::::::::::::  solution

## Solution

The following line tells `r config$sched$name` that our job should
